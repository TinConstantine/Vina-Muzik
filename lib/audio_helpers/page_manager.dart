...
class PageManager {
  ...
  Future<void> updateMediaIem(MediaItem item) async {
    return await audioHandler.updateMediaItem(item);
  }

  Future<void> moveMediaItem(
      {required int currentIndex, required int newIndex}) async {
    return await (audioHandler as AudioPlayerHandler)
        .moveQueueItem(currentIndex:  currentIndex,newIndex:  newIndex);
  }

  Future<void> removeQueueItemAt(int index) async {
    return await (audioHandler as AudioPlayerHandler).removeQueueItemAt(index);
  }
  ...
  Future<void> setRepeatMode(AudioServiceRepeatMode repeatMode) async {
    switch (repeatMode) {
      ...
      case AudioServiceRepeatMode.all:
        repeatButtonNotifier.value = RepeatState.playlist;
        break;
      case AudioServiceRepeatMode.one:
        repeatButtonNotifier.value = RepeatState.one;
        break;
  ...
  Future<void> setShuffleMode(AudioServiceShuffleMode shuffleMode) async {
    isShuffleModeEnabledNotifier.value = shuffleMode == AudioServiceShuffleMode.all;
    return audioHandler.setShuffleMode(shuffleMode);
  }
  ...
  Future<void> adds(List<MediaItem> mediaItems, int index) async {
    ...
    await (audioHandler as AudioPlayerHandler).setNewPlaylist( mediaItems: mediaItems, index: index);
  }
